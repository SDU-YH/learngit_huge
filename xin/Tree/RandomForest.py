...
class Tree(object):
    def __init__(self):
        self.split_feature = None #切分属性
        self.split_value = None #切分值
        self.leaf_value = None #类别，叶子节点才有
        self.tree_left = None  #左右子树
    ...
    def describe_tree(self):
        if not self.tree_left and not self.tree_right:  #两棵子树都为空时
...
class RandomForestClassifier(object):
    def __init__(self, n_estimators=10, max_depth=-1, min_samples_split=2, min_samples_leaf=1,
        ...
        self.min_samples_split = min_samples_split  #可分裂的最小的样本数
    ...
    def choose_best_feature(self, dataset, targets):
        ...
        for feature in dataset.columns:
            if dataset[feature].unique().__len__() <= 100:
                unique_values = sorted(dataset[feature].unique().tolist()) #返回一个排好序的新列表
            ...
            for split_value in unique_values:
                left_targets = targets[dataset[feature] <= split_value]  #收集左标签集（相应属性的取值和分裂阈值比较）
                right_targets = targets[dataset[feature] > split_value]  #收集右标签集
                split_gain = self.calc_gini(left_targets['label'], right_targets['label'])  #计算标签n的第i个可取分裂阈值的gini系数
